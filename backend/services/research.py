...
"""
    for _call in (
        lambda: router.simple_gemini(prompt),
...
async def get_weekly_market_brief(industry: str, router) -> str:
    ...
    for _call in (
        ...
        try:
            ...
            pass
    return "No notable industry news this week."
...
async def quick_search_summary(query: str, router) -> dict:
    ...
    snippets = "\n".join(f"- {i.get('title','')}: {i.get('content','')[:300]}"
                         for i in items)
    summary = ""
    _prompt = f"Summarize this in 3 sentences as a briefing for a busy executive:\n{snippets}"
    for _call in (
        lambda: router.simple_gemini(_prompt),
        lambda: router.simple_deepseek(_prompt),
        lambda: router.simple_groq(_prompt),
    ):
        try:
            summary = await _call()
            if summary:
